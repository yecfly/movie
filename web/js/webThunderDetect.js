(function(){
	try{
		...
		style.setAttribute("href", "http://www.piaohua.com/js/xl_style_6.css?197");
		headLoc.appendChild(style);
	}
	catch(e){
		document.write('<link rel="stylesheet" charset="utf-8" type="text/css" href="http://www.piaohua.com/js/xl_style_6.css?197" />');
	}
})();
/*
...
var fFXbObj;
var thunder_isIE=(navigator.userAgent.indexOf('MSIE')>0);
...
function wtd_ChangFolder(pid)
{
 ...
 else
 {
   ...
   if(selectThunderType == "coWebThunder" && mustBeSelectedThunder == true )
   {
        return "http://hezuo.down.xunlei.com/webxunlei_hezuo_1/webxl_127084.exe";
   }
   else
   {
	     return "http://hezuo.down.xunlei.com/xunlei_hezuo_1/thunder(127084).exe";
   }
...
function OnDownloadClick(sDownloadURL,sResName, sRefPage,sPid,isOpenNewWindow,sType,selectType){
  ...
  if(rand(1) == 1 && sResName!="tcfc") { //show(sDownloadURL,sPid);
...
function OnDownloadClick_Simple(linkObj,isOpenNewWindow,selectType,isFloatingLayer)
{
	 ...
	 var sDownload = "";
	 if (GetUserBrowser() == "SB360") {
	   sDownload = linkObj.getAttribute("flashHref");
	 }
	 if(sDownload == ""){
	   sDownload = linkObj.getAttribute("thunderHref");
	 }
...
if(isFloatingLayer != 5 && rand(1) == 1)
{
  productShow(sDownload,sPid,"1",linkObj,isOpenNewWindow);
    ...
    if(!thunder_isIE)
	{
       var fFXbPlugin = firefoxThunderDownload(sDownload);
	}
      if(fFXbPlugin == true)
	 {
		  fFXbObj.OpenDownloadDlg(sDownload,'http://www.155.com/?id=xb2',document.cookie);
	 }
	  else
	 {
...
{
	if(GetUserBrowser() == "SB360") {
	  linkObj.href = linkObj.getAttribute("flashHref");
	  if(linkObj.href==""){
	    linkObj.href = linkObj.getAttribute("thunderHref");
	  }
	}else{
	  linkObj.href = linkObj.getAttribute("thunderHref");
	}
...
var Thunder = {
	...
	download: function( url, refer, name, stat,cid){
	 ...
	 openWindow:function(url,flag){
		...
		if(isThunder5 == 1)
		{
			top.location.href ="http://hezuo.down.xunlei.com/xunlei_hezuo_3/thunder(127084).exe";
        }
		else
		{
			if(Info) alert(Info);
...
Thunder.Thunder5.prototype = {
	...
	if(thunderVersion != "" && thunderVersion != null)
	{
	   ...
	   {
        if(thunderVersion[0]<=7&&thunderVersion[1]<=1&&thunderVersion[2]<=5&&thunderVersion[3]<2152)
		{
...
function productShow(sDownloadURL, sPid, sFlag, linkObj, iswindowOpen){
    ...
    if (thunderObj) {
        ...
        if (!fFXbPlugin) {
            ...
            var XHR = jQuery.ajax({
                url: 'http://pingce.vip.xunlei.com/interface/check_resource6',
                data: {
	          ...
	          alert(unescape("\u62B1\u6B49\uFF0C\u60A8\u7684\u6D4F\u89C8\u5668\u4E0D\u652F\u6301\u8FC5\u96F7\u4E13\u7528\u94FE\u4E0B\u8F7D\uFF0C\u8BF7\u66F4\u6362\u522B\u7684\u6D4F\u89C8\u5668\28\u5982IE\29\u518D\u8BD5"));
                ...
                if (type == 'movie') {
                    ...
                    jQuery('#union_play_cloud').unbind('click').click(function(){
                        ...
                        window.open("http://vod.lixian.xunlei.com/share.html?from=un_" + thunder_pid + '&url=' + _url);
                    });
                } else {
                ...
                drop(jQuery('#fdiv .xunlei_lixian_hd'), 'fdiv');
                ...
                jqPopDiv.find('.xunlei_lixian_hd,.xunlei_lixian_bd').css('overflow', 'hidden');
                ...
                jqPopDiv.find('.xunlei_lixian_bar_man').css('text-align','left');
...
function DownloadByThunder(url,pid){
    ...
    if (npXLPlugin) {
        ...
        top.location.href= wtd_ChangFolder(pid);
    }
}
	...
	jQuery(document).ready(function(){
	  if(GetUserBrowser() == "SB360"){
	    jQuery("a[thunderHref]").each(function(){
	      ...
	      dllink.attr("flashHref", dllink.attr("thunderHref"));
